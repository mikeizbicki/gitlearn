...
downloadAllProjects "rshell" "$tag"
    ...
    if [ -d src ]; then
        for file in src/*; do
            if [ ! -d "$file" ] && [ "$file" != "a.out" ]; then
                local lines=$(gcc -fpreprocessed -dD -E $file 2>/dev/null | wc -l | grep -o '[[:digit:]]*')
...
# $1 = student
# $2 = commit/tag/branch location to measure
function linesOfComments {
    local loc=$(linesOfCode $1 $2)
    local locwoc=$(linesOfCodeWithoutComments $1 $2)
    echo $(($loc - $locwoc))
}
...
#echo "set xtics 86400; set grid xtics" >> scratch/out.gnu
    ...
    #set xrange [1421057599:1422057599]
    unset key
    unset ytics
...
assn0duedate="1422057599"
assn1duedate="1423439999"
duedate=$assn1duedate
startdate=$assn0duedate+86400*3
...
foreachStudent "timefirstcommit" "$startdate" > $datafile.timefirstcommit
foreachStudent "totaltime" "$startdate" "$tag" > $datafile.totaltime
...
foreachStudent "linesOfComments" "$tag" > $datafile.linesOfComments
...
paste $datafile.linesOfComments $datafile.totalGrade > $datafile.linesOfComments-vs-totalGrade
...
echo "set ylabel 'grade'; set xlabel 'lines of comments' " >> scratch/out.gnu
scatterplot $datafile.linesOfComments-vs-totalGrade >> scratch/out.gnu
...
cd scratch
gnuplot out.gnu > /dev/null 2> /dev/null
#
