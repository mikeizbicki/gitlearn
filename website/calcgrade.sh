...
giturl=$(getStudentInfo $user giturl web | sed -e 's/\.git$//')

if [ -z $giturl ]; then
    echo "<h2>Grades</h2>"
    echo "<p class=\"error\"><b>Error in getting your grades. Your GitHub URL is not in your studentinfo file.</b></p>"
    exit
fi
...
for f in `find . -name grade | sort`; do
    ...
    if [ ! $grade = "---" ]; then
        ...
        if [ "$signature" = "G" ]; then
            printf "$webgreen[signed]$webendcolor"
        else
            if [ "$signature" = "U" ]; then
                printf "$webcyn[signed but untrusted]$webendcolor"
            else
                printf "$webred[bad signature]$webendcolor"
            fi
        fi
        echo "</td>"
    else
...
dispPercentWeb "$runningpercent"
printf "</td><td class=\"grade\">"
percentToLetter "$runningpercent" "web"
echo "</td></tr><tr>"
...
dispPercentWeb "$percent"
printf "</td><td class=\"grade\">"
percentToLetter "$percent" "web"
echo "</td></tr>"
echo "</table>"
